// <copyright file="FileStorageService.cs" company="Seguridad Social API">
...
public class FileStorageService : IFileStorageService
{
    ...
    public async Task<Result<string>> SaveUploadedFileAsync(IFormFile file, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return Result<string>.Failure($"El archivo excede el tamaño máximo permitido de {options.MaxFileSizeBytes / 1_048_576} MB");
            }
            ...
            {
                return Result<string>.Failure($"Extensión de archivo no permitida. Permitidas: {string.Join(", ", options.AllowedExtensions)}");
            }
    ...
    {
        var uploadDir = Path.IsPathRooted(options.UploadDirectory)
   ? options.UploadDirectory
   : Path.Combine(Directory.GetCurrentDirectory(), options.UploadDirectory);
